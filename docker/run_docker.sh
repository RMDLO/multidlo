...
if [[ -z "${CONTAINER_DIR}" ]]; then
    CONTAINER_DIR=/root/tracking_ws/src/multidlo
fi
...
if [ -z "${TRACK_ID}" ]; then
    echo "Creating new multidlo docker container."
    xhost +local:root
    docker run  -it --privileged --network=host -v ${HOST_DIR}:${CONTAINER_DIR}:rw -v /tmp/.X11-unix:/tmp/.X11-unix:rw --env="DISPLAY" --name=${CONTAINER_NAME} rmdlo-multidlo:noetic bash
else
    echo "Found multidlo docker container: ${TRACK_ID}."
